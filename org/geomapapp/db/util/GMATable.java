...
public class GMATable extends TableDB implements Overlay {
	String title;
	XMap map;
...
//	float[][] currentXY;
	...
	int lastSelected=-1;
	public GMATable( Vector headings, Vector rows, TableDB parent) {
		super(headings, rows, parent);
	}
	public GMATable( Vector headings, Vector rows, StringBuffer comments) {
		super( headings, rows, comments);
	}
	public GMATable(String url) throws IOException {
		super( url);
	}
	public GMATable(String url, String delim) throws IOException {
		super( url, delim);
	}
	public GMATable(File file) throws IOException {
		super( file);
	}
	public GMATable(File file, String delim) throws IOException {
		super( file, delim);
	}
	public GMATable() throws IOException {
		super();
	}
	public void setTitle(String title) {
		this.title = title;
		if( shapeFile!=null ) shapeFile.setName(title);
	}
	...
	public void setMap( XMap map ) {
		this.map = map;
		if( shapeFile==null && (latCol<0 || lonCol<0) )return;
		int k=0;
...
//	System.out.println( len +"\t"+ val.length() +"\t"+ nm[0] +"\t"+ nm[len-1]);
		...
		input.close();
		GMATable table = new GMATable( names, records, (StringBuffer)null);
		return table;
	}
