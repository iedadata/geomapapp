...
public class XYMultiGraph extends XYGraph
			implements Zoomable,
				Printable,
				Scrollable	{
	...
	public XYMultiGraph( XYPoints[] pts, int dataIndex ) {
		super(pts[0], dataIndex);
	...
	private double[] getXRange() {
		...
		for (XYPoints thisxy : xyArray) {
			xmin = Math.min(xmin, thisxy.getXRange(0)[0]);
			xmax = Math.max(xmax, thisxy.getXRange(0)[1]);
		}
	...
	private double[] getYRange() {
		...
		for (XYPoints thisxy : xyArray) {
			ymin = Math.min(ymin, thisxy.getYRange(0)[0]);
			ymax = Math.max(ymax, thisxy.getYRange(0)[1]);
		}
	...
	public void paintComponent( Graphics graphics ) {
		...
		for (int i = 0; i<xyArray.length; i++) {
			xyArray[i].plotXY( g, bounds, sX, sY, dataIndex + i);
		}
