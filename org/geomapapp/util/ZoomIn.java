...
public class ZoomIn extends javax.swing.event.MouseInputAdapter {
	Point lastP;
	ScalableComp sc;
	JToggleButton toggle;
	transient Point start;
	public ZoomIn(ScalableComp sc) {
		this.sc = sc;
	}
	public JToggleButton getToggle() {
		if( toggle==null ) {
			toggle = new JToggleButton( Icons.getIcon(Icons.ZOOM_IN, false) );
			toggle.setSelectedIcon( Icons.getIcon(Icons.ZOOM_IN, true) );
	...
	public void setCursor() {
		if( toggle.isSelected() ) sc.setCursor( Cursors.getCursor( Cursors.ZOOM_IN ) );
	}
	public void mouseClicked( MouseEvent e ) {
		if( toggle.isSelected() ) sc.doZoom( e.getPoint(), 2. );
	}
