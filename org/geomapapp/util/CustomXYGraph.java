...
public class CustomXYGraph extends JComponent
			implements Zoomable,
				Printable,
				Scrollable	{
	XYPoints xy;
	...
	public CustomXYGraph( XYPoints pts, int dataIndex ) {
...
//				reset();
	...
	public void drawLineAtPoint( int currentPoint )	{
		synchronized (getTreeLock()) {
			Graphics2D g = (Graphics2D)getGraphics();
			Rectangle r = getVisibleRect();
			int x1 = r.x;
			...
			if ( prevPoint != -1)	{
				g.drawLine(x1, prevPoint, x2, prevPoint);
			}
			g.drawLine(x1, currentPoint, x2, currentPoint);
