...
public class MapServerB {
	...
	public static double[] fitWESN(double[] wesn, int width, int height) {
		...
		System.out.println("scales\t" + (1/scale) +"\t"+ (1/scaleY));
		double x0, y0;
		if(scaleY > scale) {
			scale = scaleY;
			x0 = .5d * (wesn[0] + wesn[1]);
	...
	public static boolean get3D(double[] wesn, int width, int height, BufferedImage image) {
		...
		for( int x=x1 ; x<x2 ; x++ ) {
			...
			for( int y=y1 ; y>y2 ; y--) {
				...
				String url = "file:/usr/local/seabeam/kn166-14/copy/science/stereo/g"+highRes[ires]+"/" + name;
				im = null;
				try { 
					BufferedInputStream in = new BufferedInputStream(
						(URLFactory.url(url)).openStream());
					...
					url = "file:/usr/local/seabeam/kn166-14/copy/science/tiles/g"+
						highRes[ires]+"/" + name;
					try {
						BufferedInputStream in = new BufferedInputStream(
							(URLFactory.url(url)).openStream());
	...
	public static void getImage(double[] wesn, int width, int height, BufferedImage image) {
		...
		double r = 1/scale;
		int ires = 0;
		while( 1.5*r < (double)res[ires] && ires<res.length-1) ires++;
		...
		for( int x=x1 ; x<x2 ; x++ ) {
			...
			for( int y=y1 ; y>y2 ; y--) {
				...
				String url = "file:/usr/local/seabeam/kn166-14/copy/science/tiles/g"+res[ires]+"/" + name;
				im = null;
				try { 
					BufferedInputStream in = new BufferedInputStream(
						(URLFactory.url(url)).openStream());
	...
	//	BufferedImage image = new BufferedImage(cm, ras, false, new Hashtable());
		...
		for( int x=x1 ; x<x2 ; x++ ) {
			...
			for( int y=y1 ; y>y2 ; y--) {
				...
				String url = "file:/usr/local/seabeam/kn166-14/copy/science/tiles/m"+kres+"/" + name;
				im = null;
