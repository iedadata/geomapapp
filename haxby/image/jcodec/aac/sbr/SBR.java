 ...
 * @author in-somnia
 */
public class SBR implements SBRConstants, haxby.image.jcodec.aac.syntax.SyntaxConstants, HuffmanTables {
    ...
    /* to get it compiling */
    /*
     * we'll see during the coding of all the tools, whether these are all used or
     * not.
     */
    ...
    void sbr_reset() {
        ...
        if ((this.bs_start_freq != this.bs_start_freq_prev) || (this.bs_stop_freq != this.bs_stop_freq_prev)
                || (this.bs_freq_scale != this.bs_freq_scale_prev) || (this.bs_alter_scale != this.bs_alter_scale_prev)
    ...
    int calc_sbr_tables(int start_freq, int stop_freq, int samplerate_mode, int freq_scale, boolean alter_scale,
            int xover_band) {
        int result = 0;
        int k2;
    ...
    public int decode(BitReader ld, int cnt) throws AACException {
        ...
        if (this.header_count != 0) {
            if (this.Reset || (this.bs_header_flag && this.just_seeked)) {
                int rt = calc_sbr_tables(this.bs_start_freq, this.bs_stop_freq, this.bs_samplerate_mode,
                        this.bs_freq_scale, this.bs_alter_scale, this.bs_xover_band);

                /* if an error occured with the new header values revert to the old ones */
                if (rt > 0) {
                    calc_sbr_tables(saved_start_freq, saved_stop_freq, saved_samplerate_mode, saved_freq_scale,
            ...
            if (result == 0) {
                 ...
                 * envelope_time_border_vector(). In this case the old time border vector is
                 * saved and all the previous data normally read after sbr_grid() is saved.
                 */
        ...
        if (8 * cnt < num_sbr_bits2) {
            ...
            /*
             * turn off PS for the unfortunate case that we randomly read some PS data that
             * looks correct
             */
            // this.ps_used = 0;
            ...
            while (nr_bits_left > 7) {
                ...
                if (this.bs_extension_id == EXTENSION_ID_PS) {
                    if (ps_ext_read == 0) {
                        ...
                        /*
                         * to be safe make it 3, will switch to "default" in sbr_extension()
                         */
