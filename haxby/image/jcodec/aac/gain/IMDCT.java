...
import haxby.image.jcodec.aac.syntax.ICSInfo.WindowSequence;
...
class IMDCT implements GCConstants, IMDCTTables, Windows {
        ...
        if (winSeq.equals(WindowSequence.EIGHT_SHORT_SEQUENCE)) {
            for (b = 0; b < BANDS; b++) {
                for (j = 0; j < 8; j++) {
                    for (i = 0; i < lbShort; i++) {
                        if (b % 2 == 0)
                            buf[lbLong * b + lbShort * j + i] = _in[shortFrameLen * j + lbShort * b + i];
                        else
                            buf[lbLong * b + lbShort * j + i] = _in[shortFrameLen * j + lbShort * b + lbShort - 1 - i];
                    }
                }
            }
        } else {
    ...
    private void process2(float[] _in, float[] out, WindowSequence winSeq, int winShape, int winShapePrev, int band)
            throws AACException {
        ...
        if (winSeq.equals(WindowSequence.EIGHT_SHORT_SEQUENCE)) {
            ...
            for (j = 0; j < 8; j++) {
                ...
                if (j == 0)
                    arraycopy(window1, 0, window, 0, lbShort * 2);
                else
                    arraycopy(window2, 0, window, 0, lbShort * 2);
