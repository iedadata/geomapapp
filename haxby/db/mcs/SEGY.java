...
public class SEGY {
	...
	JLabel yLabel;
	JLabel zLabel;
	public SEGY( File file ) throws IOException {
		xLabel = new JLabel("x");
		yLabel = new JLabel("y");
		zLabel = new JLabel("z");
		this.file = file;
	...
	public void process() throws IOException {
		...
		buf = new byte[nRecord*samplesPerTrace];
		int kBuf=0;
		int nRead;
	...
	if( i%100 == 0) System.out.println("trace "+i +"\tunknown = "+ unknown);
		...
		//	boolean found=true;
			...
			for( k=0 ; k<samplesPerTrace-win ; k++) {
				if(kBuf>=buf.length) {
					System.out.println("trace "+ i +"\tsample "+ k +"\t"+ kBuf +"\t"+ buf.length);
					break;
				}
			...
			//		continue;
			//	}
			//	int val = (int) ( 255. * .25 * trace[k][0]*nSum/sum);
				int val = (int) ( 340. * trace[k][0]/max);
				
				if(val<0)val=0;
				if(val>254)val=254;
				buf[k*nRecord+i] = (byte)val;
			//	if( i!=0 ) diff += Math.abs(sum/nSum - tmp[k]);
			//	tmp[k] = sum/nSum;
				if( found )continue;
			//	buf[(k-30)*nRecord+i] = 0;
				if( searching ) {
					if( trace[k][0]-trace[k-1][0] < lastDif ) {
						bottom[i] = k-1;
				//		buf[(k-2)*nRecord+i] = (byte)255;
				//		for( int kk=0 ; kk<k-20 ; kk++) buf[kk*nRecord+i]=0;
						searching = false;
						found = true;
						continue;
					} else {
	...
	public static void main(String[] args) {
		if(args.length != 1) {
			System.out.println("usage: java SEGY filename");
			System.exit(0);
		}
		try {
			File file = new File(args[0]);
			SEGY segy = new SEGY( new File(args[0]) );
			System.out.println( "SEGY Object created");
		...
		System.out.println( "processing" );
			segy.process();
			javax.imageio.ImageIO.write(segy.image, "jpg", new File(file.getParentFile(), "test.jpg"));
/*
