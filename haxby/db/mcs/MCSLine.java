...
public class MCSLine 
	...
	public MCSLine( MCSCruise cruise, String lineID) {
		this();
		this.lineID = lineID;
		this.cruise = cruise;
	}
	public MCSLine( XMap map, MCSCruise cruise, String lineID, CDP[] cdp) {
		this(cruise, lineID);
		this.map = map;
		this.cdp = cdp;
	...
	public void drawCDP( int cdpN ) {
		...
		if( cdpN==-1 ) {
			...
			shape.lineTo( (float)p.getX()-6f/zoom, (float)p.getY() );
			shape.moveTo( (float)p.getX()+2f/zoom, (float)p.getY() );
			shape.lineTo( (float)p.getX()+6f/zoom, (float)p.getY() );
			shape.moveTo( (float)p.getX(), (float)p.getY()-2f/zoom );
			shape.lineTo( (float)p.getX(), (float)p.getY()-6f/zoom);
			shape.moveTo( (float)p.getX(), (float)p.getY()+2f/zoom );
			shape.lineTo( (float)p.getX(), (float)p.getY()+6f/zoom);
		}
	...
	public void addCrossing( double cdp, double cdpX, MCSLine lineX ) {
		crossings.add( new MCSCrossing( cdp, cdpX, lineX ) );
	}
	public static double[] cross(MCSLine lineA, MCSLine lineB) {
