...
public class GeoMapAppToWorldWindTiler {
	...
	private static final String BASE = "/home/geomapapp/apache/htdocs/MapApp/merc_320_1024/";
	
	public GeoMapAppToWorldWindTiler(File outputDirectory, int levelZeroDelta, MapImageProcessor imageProcessor) {
		this.imageProcessor = imageProcessor;
		this.outputDirectory = outputDirectory;
		this.levelZeroDelta = levelZeroDelta;
	}
	
	public GeoMapAppToWorldWindTiler(File outputDirectory, int levelZeroDelta) {
	...
	public void createTilesAtLevel(final int level) throws IOException{
		...
		File[] files = FileUtility.getFiles(new File(BASE + "i_" + res), ".jpg");
		...
		for (File file : files) 
		{
			...
			Point2D p0 = proj.getRefXY(x0 - 8, y0 - 8);
			Point2D p1 = proj.getRefXY(x0 + 329, y0 + 329);
					...
					Runnable task = new Runnable() {
						public void run() {
							try {
								...
								synchronized (GeoMapAppToWorldWindTiler.this) {
...
////			}
////			
////			System.out.println(100d * tileY / tileY1);
//		}
...
//				System.out.print("\nEmpty Tile at: \t" + tileX + "," + tileY + "," + level);
		...
		if (tileImage.getHeight() != TILE_SIZE || tileImage.getWidth() != TILE_SIZE) {
			scaledTileImage = new BufferedImage(TILE_SIZE, TILE_SIZE, BufferedImage.TYPE_INT_RGB);
			scaledTileImage.createGraphics().drawImage(tileImage, 0, 0, TILE_SIZE, TILE_SIZE, null);
		} else
			scaledTileImage = tileImage;
		
		StringBuffer sb = new StringBuffer();
		sb.append(tileY).append("_").append(tileX).append(".jpg");
		
		ImageIO.write(scaledTileImage, "jpg", new File(parent, sb.toString()));
	}
...
//				System.out.println(Math.toDegrees(lat) + "\t" + sampleY + "\t" + sampleRatio);
	...
	public static void main(String[] args) throws IOException{
		if (args.length < 1) {
			System.err.println("Usage: GridToWorldWindTiler outputDirectory [TileLevel]");
			System.exit(-1);
		}
		
		GeoMapAppToWorldWindTiler tiler = new GeoMapAppToWorldWindTiler(new File(args[0]), 36);
